...
func testMailBoxDust(t *testing.T, chantype channeldb.ChannelType) {
	...
	// creations.
	ctx.mailbox.SetFeeRate(chainfee.AtomPerKByte(253))

	localDustLimit := dcrutil.Amount(400)
	remoteDustLimit := dcrutil.Amount(500)
	...
	// htlc channel type.
	firstAmt := lnwire.MilliAtom(600_000)

	if chantype.ZeroHtlcTxFee() {
		firstAmt = lnwire.MilliAtom(450_000)
	}
	...
	localSum, remoteSum = ctx.mailbox.DustPackets()
	require.Equal(t, lnwire.MilliAtom(0), localSum)
	require.Equal(t, lnwire.MilliAtom(0), remoteSum)
}
